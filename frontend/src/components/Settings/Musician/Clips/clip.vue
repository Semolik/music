<template>
    <form class="add-clip-container">
        ...
        <div class="clip-content">
            ...
            <Checkbox
                v-model="saveImageFromUrl"
                label="Использовать превью ролика как изображние"
            />
            <ButtonsBlock
                :wrong="wrong"
                :active="buttonActive"
...
export default {
    ...
    async setup(props) {
        ...
        if (props.id) {
            const { data } = await HTTP.get("/clips/clip", {
                params: { id: props.id },
            });
            clipData = data;
        ...
        return {
            VITE_MAX_CLIP_NAME_LENGTH: Number(VITE_MAX_CLIP_NAME_LENGTH),
            clipData,
        };
    },
    props: {
    ...
    methods: {
        ...
        async create(form) {
            const {
                data: { id },
            } = await HTTP.post("/clips/clip", form);
    ...
    computed: {
        youtubeId() {
            var result =
                /http(?:s?):\/\/(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)([\w\-\_]*)(&(amp;)?‌​[\w\?‌​=]*)?/.exec(
                    this.url
                );
        ...
        buttonActive() {
            ...
            return (
                (this.url !== this.originalClipData.video ||
                    this.name !== this.originalClipData.name ||
                    this.notEmpty ||
                    this.image) &&
                !this.wrong
            );
