<template>
    ...
    <div class="clip-buttons">
        <div class="load-more" @click="getNextPage" v-if="isbuttonShowed">
            Загрузить еще
        </div>
    </div>
</template>
...
library.add(faPlus);
export default {
    components: { FontAwesomeIcon, ClipsList },
    async setup() {
        const { getNextPage, clips, isbuttonShowed } = useClipsGetter(
            null,
            true,
            true
        );
    ...
    computed: {
        filteredClips() {
            ...
            return this.clips.filter((clip) =>
                clip.name.toLowerCase().includes(this.text.toLowerCase())
            );
        },
    },
};
