...
def create_genre(
    ...
    if genres_cruds.get_genre_by_name(name=genreData.name):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail="Название жанра должно быть уникальным")
    db_image = save_image(upload_file=genrePicture,
                          user_id=current_user_id)
    db_genre = genres_cruds.create_genre(name=genreData.name, picture=db_image)
...
def update_genre(
    ...
    current_user_id = Authorize.get_jwt_subject()
    validate_admin(user_id=current_user_id)
    genre = genres_cruds.get_genre_by_id(id=genreData.id)
    if not genre:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Жанр не найден")
    db_image = save_image(upload_file=genrePicture,
                          user_id=current_user_id)
    genre = genres_cruds.update_genre(
...
def delete_genre(id: int, Authorize: AuthJWT = Depends()):
    ...
    if not genre:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Жанр не найден")
    genre = genres_cruds.detete_genre(genre_id=id)
