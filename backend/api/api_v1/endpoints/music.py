...
from backend.helpers.music import save_track, set_album_info, set_album_tracks
...
from backend.schemas.track import AlbumAfterUpload, AlbumInfo, CreateAlbumForm, CreateGenre, CreateGenreForm, Genre, TrackAfterUpload, UpdateGenreForm, UploadTrackForm
...
def create_album(albumData: CreateAlbumForm = Depends(CreateAlbumForm), albumPicture: UploadFile = File(default=False), Authorize: AuthJWT = Depends()):
    ...
    genres = []
    genres_ids = albumData.genres_ids
    if genres_ids:
...
@router.get('/album', responses={**NOT_FOUND_ALBUM}, response_model=AlbumInfo)
