...
class MusicianCrud(CRUDBase):
    ...
    def get_popular_musician_tracks(self, musician_id: int, page: int, page_size: int = 10) -> List[Album]:
        end = page * page_size
        return self.db.query(Track)\
            .filter(Track.artist_id == musician_id)\
            .join(ListenTrackHistoryItem)\
            .group_by(Track.id) \
